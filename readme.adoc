...
.build.mill.yaml ({mill-download-url}/mill-dist-{mill-version}-example-scalalib-basic-1-simple.zip[download], {mill-example-url}/example/scalalib/basic/1-simple/build.mill.yaml[browse])
...
.test/package.mill.yaml ({mill-download-url}/mill-dist-{mill-version}-example-scalalib-basic-1-simple.zip[download], {mill-example-url}/example/scalalib/basic/1-simple/test/package.mill.yaml[browse])
[source,yaml,subs="attributes,verbatim"]
----
...
This uses Mill's declarative `.mill.yaml` files for defining the
modules:
...
NOTE: `build.mill.yaml` and `package.mill.yaml` files import `build.\*` and
`{language-small}lib.*` for convenience, allowing us to write `{language}Module`
and `TestModule` un-qualified in the example above. You can also refer to it fully-qualified via
`mill.{language-small}lib.{language}Module` and `mill.{language-small}lib.TestModule`.
...
[source,console]
----
> ./mill repl    # start a Scala REPL within your project
> ./mill test.repl

----

You can also start a standalone Scala REPL via `./mill repl` from any directory,
even without a `build.mill` file present.
...
This example uses Mill's declarative YAML syntax, which is a good fit for simple builds
where you are just setting some configuration keys. This is discussed more in the documentation
for:

* xref:{language-small}lib/module-config.adoc#_declarative_configuration[Declarative {language} Configuration]
